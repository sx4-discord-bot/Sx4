...
public class FishingRodCommand extends Sx4Command {
	...
	public void repair(Sx4CommandEvent event, @Argument(value="durability") @AlternativeOptions("all") Alternative<Integer> option) {
		...
		String acceptId = new CustomButtonId.Builder()
			.setType(ButtonType.FISHING_ROD_REPAIR_CONFIRM)
			.setTimeout(60)
			.setOwners(event.getAuthor().getIdLong())
			.setArguments(item.getId(), rod.getId(), rod.getDurability(), durability)
			.getId();
		...
		event.reply("It will cost you `" + itemCount + " " + item.getName() + "` to repair your fishing rod by **" + durability + "** durability, are you sure you want to repair it?")
