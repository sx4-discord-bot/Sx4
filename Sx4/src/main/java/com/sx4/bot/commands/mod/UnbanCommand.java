...
public class UnbanCommand extends Sx4Command {
	...
	public void onCommand(CommandEvent event, @Argument(value="user") String userArgument, @Argument(value="reason", endless=true, nullDefault=true) String reason) {
		SearchUtility.getUserRest(userArgument, user -> {
			...
			}, new ErrorHandler().handle(ErrorResponse.UNKNOWN_BAN, e -> event.reply("That user is not banned :no_entry:").queue()));
		});
	}
