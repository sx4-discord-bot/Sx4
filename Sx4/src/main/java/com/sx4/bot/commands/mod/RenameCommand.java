...
public class RenameCommand extends Sx4Command {
	...
	public void onCommand(Sx4CommandEvent event, @Argument(value="user", nullDefault=true) Member member, @Argument(value="nickname", endless=true) @Limit(max=32) String nick) {
		...
		event.getGuild().modifyNickname(effectiveMember, nick)
			.flatMap($ -> event.replySuccess("Renamed " + effectiveMember.getAsMention() + " to **" + nick + "**"))
			.queue();
	}
