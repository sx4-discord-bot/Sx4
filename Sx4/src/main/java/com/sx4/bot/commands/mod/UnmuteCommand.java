...
public class UnmuteCommand extends Sx4Command {
	...
	public void onCommand(CommandEvent event, @Context Database database, @Argument(value="user") Member member, @Argument(value="reason", endless=true, nullDefault=true) Reason reason) {
		long roleId = database.getGuildById(event.getGuild().getIdLong(), Projections.include("mute.roleId")).getEmbedded(List.of("mute", "roleId"), 0L);
		...
		database.updateGuildById(event.getGuild().getIdLong(), Updates.pull("mute.users", Filters.eq("id", member.getIdLong()))).whenComplete((result, exception) -> {
			if (exception != null) {
