...
public class PickaxeCommand extends Sx4Command {
	...
	public void buy(Sx4CommandEvent event, @Argument(value="pickaxe", endless=true) Pickaxe pickaxe) {
		...
		Document pickaxeData = event.getMongo().getItem(filter, Projections.include("_id"));
		if (pickaxeData != null) {
		...
		event.getMongo().findAndUpdateUserById(event.getAuthor().getIdLong(), List.of(EconomyUtility.getBalanceUpdate(pickaxe.getPrice())), options).thenCompose(data -> {
			...
			return event.getMongo().insertItem(insertData);
	...
	public void repair(Sx4CommandEvent event, @Argument(value="durability") @Options("all") Alternative<Integer> option) {
		...
		event.reply("It will cost you `" + itemCount + " " + item.getName() + "` to repair your pickaxe by **" + durability + "** durability, are you sure you want to repair it? (Yes or No)").submit().thenCompose($ -> {
			...
			List<Bson> update = List.of(
				Operators.set("item.currentDurability", Operators.cond(Operators.eq("$item.currentDurability", pickaxe.getCurrentDurability()), Operators.add("$item.currentDurability", durability), "$item.currentDurability")),
				Operators.set("item.price", Operators.add("$item.price", Operators.multiply(pickaxe.getPrice() / pickaxe.getMaxDurability(), durability)))
			);
