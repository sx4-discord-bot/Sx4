...
public class PickaxeCommand extends Sx4Command {
	...
	public void repair(Sx4CommandEvent event, @Argument(value="durability") @AlternativeOptions("all") Alternative<Integer> option) {
		...
		String acceptId = new CustomButtonId.Builder()
			.setType(ButtonType.PICKAXE_REPAIR_CONFIRM)
			.setTimeout(60)
			.setOwners(event.getAuthor().getIdLong())
			.setArguments(item.getId(), pickaxe.getId(), pickaxe.getDurability(), durability)
			.getId();
		...
		event.reply("It will cost you `" + itemCount + " " + item.getName() + "` to repair your pickaxe by **" + durability + "** durability, are you sure you want to repair it?")
