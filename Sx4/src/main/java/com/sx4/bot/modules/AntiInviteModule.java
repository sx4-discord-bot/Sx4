...
public class AntiInviteModule {
	...
	public class AntiInviteCommand extends Sx4Command {
		...
		public void toggle(CommandEvent event, @Context Database database) {
			List<Bson> update = List.of(Aggregates.addFields(new Field<>("antiinvite.enabled", Conditions.cond("$antiinvite.enabled", "$$REMOVE", true))));
			database.getGuildByIdAndUpdate(event.getGuild().getIdLong(), update, Projections.include("antiinvite.enabled"), (data, exception) -> {
				if (exception != null) {
					exception.printStackTrace();
					event.reply(Sx4CommandEventListener.getUserErrorMessage(exception)).queue();
				} else {
					event.reply("Anti-Invite is now " + (data.getEmbedded(List.of("antiinvite", "enabled"), false) ? "enabled" : "disabled") + " in this server <:done:403285928233402378>").queue();
				}
			});
		}
