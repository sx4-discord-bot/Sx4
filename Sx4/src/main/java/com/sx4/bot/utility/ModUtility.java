	...
	public static CompletableFuture<Warn> warn(Member target, Member moderator, Reason reason) {
		...
		database.findAndUpdateGuildById(guild.getIdLong(), update, options).whenComplete((data, exception) -> {
			...
			switch (action.getModAction()) {
				...
				case MUTE:
				case MUTE_EXTEND:
					if (!guild.getSelfMember().hasPermission(Permission.MANAGE_ROLES)) future.completeExceptionally(new BotPermissionException(Permission.MANAGE_ROLES));
					...
					break;
				case KICK:
					if (!guild.getSelfMember().hasPermission(Permission.KICK_MEMBERS)) future.completeExceptionally(new BotPermissionException(Permission.KICK_MEMBERS));
					if (!guild.getSelfMember().canInteract(target)) future.completeExceptionally(new BotHierarchyException("I cannot kick a user higher or equal than my top role"));
					if (!moderator.hasPermission(Permission.KICK_MEMBERS)) future.completeExceptionally(new AuthorPermissionException(Permission.KICK_MEMBERS));
					...
					break;
				case TEMP_BAN:
					if (!guild.getSelfMember().hasPermission(Permission.BAN_MEMBERS)) future.completeExceptionally(new BotPermissionException(Permission.BAN_MEMBERS));
					if (!guild.getSelfMember().canInteract(target)) future.completeExceptionally(new BotHierarchyException("I cannot ban a user higher or equal than my top role"));
					if (!moderator.hasPermission(Permission.BAN_MEMBERS)) future.completeExceptionally(new AuthorPermissionException(Permission.BAN_MEMBERS));
